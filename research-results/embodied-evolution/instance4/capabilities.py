...
def load_connectome():
    """Load the FlyWire connectome as DataFrame with a unique positional index."""
...
def scale_brain(df, factor, seed=42):
    ...
    if factor < 1.0:
        max_neuron = max(df[pre_col].max(), df[post_col].max())
        keep = set(random.sample(range(int(max_neuron) + 1), int(max_neuron * factor)))
...
# ── Module-level snapshot for Hebbian-drift-free neural evaluation ────────────
_BIO_SYN_SNAPSHOT = None  # Clean biological weights before any Hebbian modification


def run_neural_only(df, n_steps=300, stimulus='sugar', seed=42):
    """
    Run ONLY the brain (no physics body). Fast: ~17s per 300 steps.

    Restores weights from a pre-Hebbian biological snapshot before each call
    so results are reproducible regardless of call order.

    Returns dict with feed_rate, forward_rate, turning_bias, escape_rate,
    and all_dn_rates. fitness = feed_rate (for sugar stimulus).
    """
    global _BRAIN_CACHE, _ORIG_DF_CACHE, _BIO_SYN_SNAPSHOT
    import torch
    from brain_body_bridge import BrainEngine, DNRateDecoder, DN_GROUPS

    np.random.seed(seed)

    if _BRAIN_CACHE is None:
        _BRAIN_CACHE = BrainEngine(device='cpu')
        _bio = pd.read_parquet(
            '/home/ubuntu/fly-brain-embodied/data/2025_Connectivity_783.parquet')
        _ORIG_DF_CACHE = _bio['Excitatory x Connectivity'].values.astype(np.float64)
        del _bio
        # Snapshot clean biological weights (before any Hebbian drift)
        _BIO_SYN_SNAPSHOT = _BRAIN_CACHE._syn_vals.clone()

    brain = _BRAIN_CACHE

    # Restore biological baseline (undo Hebbian drift from previous calls)
    if _BIO_SYN_SNAPSHOT is not None:
        brain._syn_vals.copy_(_BIO_SYN_SNAPSHOT)

    # Apply mutation delta on top of clean biological weights
    _inject_connectome_weights(brain, df, _ORIG_DF_CACHE)
    ...
    all_dn_rates = {name: decoder.get_normalized(name) for name in brain.dn_indices}

    forward_rate = float(np.mean([
        all_dn_rates.get('P9_left', 0), all_dn_rates.get('P9_right', 0),
        all_dn_rates.get('P9_oDN1_left', 0), all_dn_rates.get('P9_oDN1_right', 0),
    ]))
    turn_left  = (all_dn_rates.get('DNa01_left', 0) +
                  0.5 * all_dn_rates.get('DNa02_left', 0))
    turn_right = (all_dn_rates.get('DNa01_right', 0) +
                  0.5 * all_dn_rates.get('DNa02_right', 0))
    turning_bias  = float(turn_left - turn_right)
    feed_rate     = float(np.mean([all_dn_rates.get('MN9_left', 0),
                                   all_dn_rates.get('MN9_right', 0)]))
    escape_rate   = float(np.mean([all_dn_rates.get('GF_1', 0),
                                   all_dn_rates.get('GF_2', 0)]))

    return {
        'fitness':       feed_rate,
        'forward_rate':  forward_rate,
        'turn_left':     float(turn_left),
        'turn_right':    float(turn_right),
        'turning_bias':  turning_bias,
        'feed_rate':     feed_rate,
        'escape_rate':   escape_rate,
        'all_dn_rates':  all_dn_rates,
        'n_steps':       n_steps,
        'stimulus':      stimulus,
    }


def mutate_targeted(df, target_flyids, n_mutations=100, seed=None,
                    weight_range=0.3, n_hops=1, frac_targeted=0.8):
    """
    Mutate synapses preferentially in the pathway TO target neurons.
    target_flyids: list of FlyWire IDs (e.g. MN9 FlyWire IDs)
    n_hops: how many hops upstream to include (1 = direct pre-synaptic)
    """
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    col_pos  = df.columns.get_loc(weight_col)

    target_set = set(target_flyids)
    for _ in range(n_hops):
        upstream = set(df.loc[df[post_col].isin(target_set), pre_col].tolist())
        target_set |= upstream

    pathway_pos = list(np.where(
        df[post_col].isin(target_set) | df[pre_col].isin(target_set)
    )[0])
    all_pos = list(range(len(df)))

    n_targeted = int(n_mutations * frac_targeted)
    n_random   = n_mutations - n_targeted

    chosen_t = random.sample(pathway_pos, min(n_targeted, len(pathway_pos))) if pathway_pos else []
    chosen_r = random.sample(all_pos,     min(n_random,   len(all_pos)))
    chosen   = list(set(chosen_t + chosen_r))

    for pos in chosen:
        current = float(df.iat[pos, col_pos])
        change  = np.random.uniform(-weight_range, weight_range) * (abs(current) + 1.0)
        df.iat[pos, col_pos] = float(current + change)
