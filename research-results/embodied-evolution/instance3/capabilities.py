...
def mutate(df, n_mutations=5, seed=None, strategy="mixed", weight_range=0.2, rewire_radius=500):
    ...
    weight_col_pos = df.columns.get_loc(weight_col)

    mutation_details = []
    for idx in chosen:
        if strategy == "weight" or (strategy == "mixed" and random.random() < 0.9):
            old_val = float(df.iat[idx, weight_col_pos])
            change = np.random.uniform(-weight_range, weight_range) * abs(old_val)
            new_val = max(0, old_val + change)
            # CRITICAL: use iat (positional) not at (label-based).
            # df.index has only ~26K unique labels across 15M rows; df.at[label]
            # updates ALL ~1000 rows with that label. df.iat[pos] = single row.
            df.iat[idx, weight_col_pos] = new_val
...
def run_embodied(df, duration_sec=2.0, seed=42, stimulus='sugar', warmup_steps=500):
    """
    ...
    Args:
        ...
        warmup_steps: Brain steps before physics (sugar stimulus needs ~415 steps
                      to propagate to DN neurons; without warmup DNs never fire)
    ...
    # ── Brain warmup: pre-run brain steps before physics ─────────────────────
    # CRITICAL: sugar stimulus takes ~415 brain steps (41ms) to propagate to DNs.
    # The 2s simulation only runs 200 brain steps (200 x BRAIN_RATIO=100 physics steps
    # at dt=1e-4). Without warmup, DNs never fire → drive=[0,0] → pure CPG → all
    # connectomes look identical. warmup_steps=500 establishes steady-state DN activity.
    if warmup_steps > 0:
        brain._hebb_count = -999999  # Suppress Hebbian during warmup
        for _ in range(warmup_steps):
            brain.step()
            s = brain.get_dn_spikes()
            decoder.update(s, None)
        brain._hebb_count = 0  # Resume Hebbian for main simulation
...
def scale_brain(df, factor, seed=42):
    ...
    # CRITICAL: must use *_Index columns (range 0-138638), NOT *_ID columns
    # (range 720e12). random.sample(range(720e12+1), ...) causes MemoryError.
    pre_col = 'Presynaptic_Index'
    post_col = 'Postsynaptic_Index'

    if factor < 1.0:
        max_neuron = max(df[pre_col].max(), df[post_col].max())
        keep = set(random.sample(range(int(max_neuron) + 1), int((max_neuron + 1) * factor)))
